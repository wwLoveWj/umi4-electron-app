...
import EmailSendDrawer from "./EmailSendDrawer";
...
const EmailRecordList = forwardRef<EmailRecordListRef>((_, ref) => {
  ...
  return (
    <>
        ...
        extra={
          <Space>
            <Search
              ...
              style={{ width: "300px" }}
            />
            <Button type="primary" onClick={() => setSendDrawerOpen(true)}>
              发送邮件
            </Button>
          </Space>
        }
      ...
      {/* 发送邮件抽屉 */}
      <EmailSendDrawer
        value={sendDrawerOpen}
        onChange={setSendDrawerOpen}
        onSuccess={async (newTaskId?: string) => {
          // 如果有新的 taskId，说明是新添加的记录，直接添加到列表开头
          if (newTaskId) {
            try {
              // 获取新添加的记录
              const newRecord =
                await indexedDBUtil.getEmailRecordByTaskId(newTaskId);
              if (newRecord) {
                // 将新记录添加到列表开头
                ...
                if (!searchText.trim()) {
                  ...
                  const filtered = newRecords.filter((record) =>
                    record.subject
                      .toLowerCase()
              ...
              console.error("获取新记录失败:", error);
              // 如果获取新记录失败，回退到完整刷新
              fetchRecords();
            }
          } else {
            // 没有 taskId 的情况，可能是其他类型的更新，使用完整刷新
            fetchRecords();
          }
