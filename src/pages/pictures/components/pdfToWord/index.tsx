...
export default function Index() {
  ...
  React.useEffect(() => {
    ...
    ipcRenderer.on("pdf-to-word-progress", handleProgress);

    return () => {
      ipcRenderer.removeListener("pdf-to-word-progress", handleProgress);
    };
  }, []);
  ...
  const handleFiles = async (files: File[]) => {
    try {
      ...
      // 转换 PDF 到 Word
      const result = await ipcRenderer.invoke("convert-pdf-to-word", {
        pdfs: pdfData,
        savePath,
      });
  ...
  return (
    <div className={styles.pdfToWordBox}>
      <h3>PDF 转 Word 文档</h3>
